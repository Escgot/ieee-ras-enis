      ...
      <div className="relative w-full z-10">
        ...
        <div className="px-4 sm:px-6 lg:px-12 max-w-[1700px] mx-auto w-full">
          <div className="relative">
            ...
            {/* ── MOBILE: Simple horizontal CSS scroll (no JS carousel) ── */}
            {isMobile && (
              <div className="flex overflow-x-auto gap-4 py-8 no-scrollbar snap-x snap-mandatory">
                {teamMembers.map((member, index) => (
                  <div
                    key={index}
                    className="flex-[0_0_260px] snap-center"
                  >
                    <div className="relative aspect-[3/4] rounded-2xl overflow-hidden">
                      ...
                      <div className="absolute inset-x-0 bottom-0 p-5">
                        ...
                        <h4 className="font-orbitron text-lg font-black text-white uppercase leading-tight mb-3">
                          {member.name}
                        </h4>
                        <div className="flex gap-2">
                          {[
                            { href: member.social.instagram, Icon: Instagram },
                            { href: member.social.linkedin, Icon: Linkedin },
                            { href: member.social.facebook, Icon: Facebook },
                            { href: member.social.mail, Icon: Mail },
                          ].map(({ href, Icon }, i) => (
                            <a
                              key={i}
                              href={href}
                              ...
                              className="w-7 h-7 flex items-center justify-center rounded-lg bg-white/10 border border-white/10 text-gray-400 hover:text-white transition-colors"
                            >
              ...
              <div>
                  ...
                  <div className="flex flex-row" style={{ backfaceVisibility: 'hidden' }}>
                    {teamMembers.map((member, index) => {
                      ...
                      return (
                        <div
                          key={index}
                          className="flex-[0_0_auto] min-w-0 px-5 cursor-pointer"
                          onClick={() => emblaApi?.scrollTo(index)}
                        >
                          <div
                            className={`relative flex-shrink-0 w-[320px] aspect-[3/4] rounded-[2rem] overflow-hidden transition-all duration-700 group will-change-transform ${isActive
                              ? 'scale-100 opacity-100 z-20 -translate-y-2'
                              : 'scale-[0.85] opacity-40 z-10'
                              }`}
                            style={{
                          ...
                          >
                            ...
                            <div className={`absolute inset-x-0 bottom-0 p-6 transition-opacity duration-500 z-20 ${isActive ? 'group-hover:opacity-0 opacity-100' : 'opacity-100'}`}>
                            ...
                            {isActive && (
                              <div className="absolute inset-x-0 bottom-0 p-6 opacity-0 group-hover:opacity-100 transition-all duration-500 z-30 translate-y-2 group-hover:translate-y-0">
                                <div className="bg-black/80 border border-white/10 rounded-2xl p-5">
                                  ...
                                  <h4 className="font-orbitron text-xl font-black text-white mb-4 uppercase leading-tight">
                                    {member.name}
                                  </h4>
                ...
                <div className="flex items-center justify-center gap-6 -mt-4">
                  <button
                    ...
                    className="w-12 h-12 flex items-center justify-center rounded-full border border-white/10 hover:border-red-500/50 bg-white/5 text-gray-400 hover:text-red-400 transition-all disabled:opacity-30"
                    aria-label="Previous"
                  >
